...
function Orders() {
  ...
  useEffect(() => {
    void api.get<OrderRow[]>('/api/account/orders').then(setRows);
  }, []);
  return (
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-(--color-border)">
                ...
                <td className="px-4 py-3 text-right">{money(r.totalCents, r.currency)}</td>
                <td className="px-4 py-3 text-right">
                  <Link to={`/orders/${r.id}`} className="text-(--color-accent) font-semibold">View</Link>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  return (
    ...
    >
      ...
      <label className="block text-sm">Current password
        <input type="password" className="input mt-1" value={currentPassword} onChange={(e) => setCurrent(e.target.value)} autoComplete="current-password" />
      </label>
      <label className="block text-sm">New password
        <input type="password" className="input mt-1" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="new-password" />
      </label>
        ...
        <nav className="flex lg:flex-col gap-1 text-sm">
          {[
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                'px-3 py-2 rounded-md ' + (isActive ? 'bg-(--color-accent-soft) text-(--color-accent) font-semibold' : 'hover:bg-(--color-accent-soft)/50')
              }
            >
              {t.label}
            </NavLink>
