...
interface Stats {
  ...
  recentOrders: { id: string; orderNumber: string; status: string; totalCents: number; currency: string; createdAt: string | number | Date }[];
...
export default function Dashboard() {
  ...
  useEffect(() => {
    void api.get<Stats>('/api/admin/stats').then(setS);
  }, []);
  if (!s) return <div className="grid place-items-center py-32"><div className="spinner" /></div>;
  return (
          ...
          <ul className="space-y-1 text-sm">
            {s.lowStock.map((p) => (
              <li key={p.id} className="flex justify-between">
                <Link to={`/admin/products/${p.id}/keys`} className="hover:text-(--color-accent)">{p.name}</Link>
                <span className="text-(--color-danger) font-semibold">{p.available} left</span>
              </li>
            ))}
          </ul>
        )}
